...
class DriverMonitoringSystem:
    def __init__(self):
        ...
        self.EYES_CLOSED_THRESHOLD = 1.5  
        self.YAWNING_THRESHOLD = 1.0     
    ...
    def process_phone_detection(self, frame, results, ear_coords, yolo_results=None):
        ...
        return phone_detected, phone_near_ear, hand_near_ear
    ...
    def process_head_position(self, frame, results):
        ...
        if results.face_landmarks:
            try:
                ...
                # Detect head position - with increased thresholds
                if y_angle < -30:  # Changed from -20 to -30
                    head_position = "Looking Left"
                elif y_angle > 30:  # Changed from 20 to 30
                    head_position = "Looking Right"
                elif x_angle < -30:  # Changed from -20 to -30
    ...
    def update_status_overlay(self, frame, status_data, phone_detected, phone_near_ear, hand_near_ear):
        """Create a persistent overlay for status display with improved phone detection messaging"""
        h, w = frame.shape[:2]
        ...
        # Improved phone detection messaging
        if status_data['phone_use']:
            if phone_near_ear or hand_near_ear:
                cv2.putText(overlay, "On Call", (30, y_offset),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 255), 2)
            elif phone_detected:
                cv2.putText(overlay, "Phone Usage Detected", (30, y_offset),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 255), 2)
            else:
    ...
    def run(self):
        ...
        while True:
            ...
            phone_detected, phone_near_ear, hand_near_ear = self.process_phone_detection(
            ...
            phone_use = phone_detected or phone_near_ear or hand_near_ear
            self.detections['phone_use'] = phone_use
            ...
            if current_time - self.last_status_update_time >= self.STATUS_UPDATE_INTERVAL:
                self.status_overlay = self.update_status_overlay(frame, self.detections, phone_detected, phone_near_ear, hand_near_ear)
